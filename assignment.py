...
# The value by how much the speed limit can be exceeded 
EXCEED = 30

# The distance of any edge
DISTANCE = 100

# This value sets which budget you want to plot the path for in Question 3
PRINT_BUDGET = 300

# These are util functions to set the speed limits and fines
...
# This function is the main optimisation program using Gurobi
...
# A simple util function to find the index of an element in an array
...
# This function plots the path obtained by the objective function
# If the speed limit is broken on an edge, it is highlighted as red and marked with the corresponding speed limit
#
## Note: This function may not work in the case of the linear programming relaxation
...
# This function prints out the results of each of the variables once the optimisation is complete
...
#########################################################################################
### Below is the code that is run to solve and answer each of the four questions ###
#########################################################################################

### Question 1###
result = optimise(0, relax=False); # Find the path with budget of 0
resultB = optimise(-1, relax=False); # Find the path with infinite budget 
print("Q1", result);
print("Q1B", resultB);
...
for budget in budgets:
    result = optimise(budget, relax=False); # Find the path for all budgets in the range defined (incrementing by 50)
    costs.append(result[1]);
    nodes.append(result[3]);
    sumTimes = result[2];

    if budget == PRINT_BUDGET:  # This will only plot the path with budget equal to PRINT_BUDGET
        plotPath(result[0],budget,'img/path_budget_' + str(budget) + ".png")

    for run in range(0, NUMRUNS - 1): # Averaging the running time of budgets over multiple runs
        result = optimise(budget, relax=False);
        sumTimes += result[2];
    times.append(sumTimes / NUMRUNS * 1000);

### Question 3 ###
fig, axs = plt.subplots(3); # Plot for each of the budgets the travel time, running time and number of nodes in the Branch and Bound tree
...
### Question 4 ###
result = optimise(350, relax=True); # Find the optimal solution for the linear programming relaxation problem.
